...
if __name__ == '__main__':
    ...
    print("🏆 最终级联验证结果 (MAE)")
    print("=" * 50)
    print(f"🍊 盲测模型 (Blind)      : {mae_blind:.4f} C")
    print(f"🚀 虚拟传感 (Virtual Full): {mae_virtual:.4f} C")
    print(f"🌲 理想全参 (Ideal Full)  : {mae_ideal:.4f} C")
    print("-" * 50)
    print(f"🎯 实际功率分类准确率: {class_acc:.2f}%")

    # --- 📊 绘图 (修改部分) ---
    # ... (前面的代码保持不变) ...

    # --- 📊 绘图 (修改部分：添加准确率标注) ---
    plt.figure(figsize=(9, 7))  # 稍微调高一点高度
    methods = ['Blind', 'Virtual Full', 'Ideal Full']
    maes = [mae_blind, mae_virtual, mae_ideal]
    colors = ['#FFA500', '#DC143C', '#228B22']  # Orange, Crimson, ForestGreen
    ...
    # ✨✨✨ 2. 新增：在图表中添加分类准确率“勋章” ✨✨✨
    # transform=plt.gca().transAxes 意味着使用相对坐标 (0~1)
    # x=0.05, y=0.92 大概在左上角
    info_text = f"Power Classification Accuracy: {class_acc:.2f}%"
    ...
    plt.tight_layout()
    plt.savefig("虚拟仪器测试集结果图.png", dpi=300)
    print("📊 验证对比图已保存: 虚拟仪器测试集结果图.png (含准确率标注)")